from flask import Flask, render_template, session, url_for, session,\
...
try:
    con = psycopg2.connect(dbname='agri',user='navin',host='localhost',password='8384')
...
def search_year():
    ...
    return render_template('search_year.html',state_data = state_data,\
        district_data=district_data,commodity_data=commodity_data,
...
def search_record():
    ...
    return render_template('search_record.html',state_data = state_data,\
        district_data=district_data,market_data=market_data)
...
def search_location():
    ...
    con.commit()
    cur.execute("select distinct extract(year from arrival_date)::int as year\
     from daily_prices order by year")
    ...
    return render_template('search_location.html',state_data = state_data,\
        district_data=district_data,year_data=year_data,commodity_data=commodity_data)
...
def search_results():
    ...
    if request.method == 'POST':
        ...
        sql = "select d.year::int , round(max(d.max_price)::numeric,2)::int,\
            round(min(d.min_price)::numeric,2)::int,\
            round(avg(d.modal_price)::numeric,2)::int\
            from (select * from state where state_name = %s) st inner join\
            (select * from district where district_id = %s)\
            dt on dt.state_id = st.state_id inner join\
            market m on m.district_id = dt.district_id inner join\
            (select record_id, market_id, variety_id,extract (year from arrival_date)\
            as year, min_price, max_price,modal_price from daily_prices where\
            min_price > 0 and max_price > 0 and modal_price > 0) d on\
            d.market_id = m.market_id inner join variety v on\
            v.variety_id = d.variety_id inner join (select * from\
            commodity where commodity_name = %s) c on\
            c.commodity_id = v.commodity_id group by d.year order by d.year;"
        con.commit()
...
def search_results_record():
    ...
    if request.method == 'POST':
        ...
        print(market_name)
        sql = "select commodity.commodity_name, variety.variety_name,\
            daily_prices.arrival_date, daily_prices.max_price::int,\
            daily_prices.min_price::int, daily_prices.modal_price::int \
            from daily_prices \
    ...
    return render_template('search_results_record.html',data=data,state=state,
            district=district_name[0],market=market_name[0])
...
def search_results_year():
    ...
    if request.method == 'POST':
        ...
        if variety == "All":
            print("Hello")
            sql = "with d as (select record_id,market_id, daily_prices.variety_id,\
                extract (year from arrival_date) as year, min_price, max_price,modal_price\
                from daily_prices\
                ...
                where market_id in (select market.market_id from market where market.district_id = %s)\
                and min_price > 0 and max_price > 0 and modal_price > 0)\
                select d.year::int , round(max(d.max_price)::numeric,2)::int,\
                round(min(d.min_price)::numeric,2)::int,\
                round(avg(d.modal_price)::numeric,2)::int from d group by d.year\
                order by d.year;"
            con.commit()
...
def search_results_location():
    ...
    if request.method == 'POST':
        ...
        if state==None:
            sql = "select st.state_name, dt.district_name, d.year::int,\
            round(max(d.max_price)::numeric,2)::int,\
            round(min(d.min_price)::numeric,2)::int ,\
            round(avg(d.modal_price)::numeric,2)::int \
            from state st \
            inner join district dt on dt.state_id = st.state_id \
            inner join market m on m.district_id = dt.district_id \
            inner join (select record_id, market_id, variety_id,\
            extract (year from arrival_date) as year, min_price,\
            max_price,modal_price from daily_prices \
            ...
            inner join (select * from commodity where commodity_name = %s)\
            c on c.commodity_id = v.commodity_id \
...
def market():
    ...
    return render_template('market.html',state_data = state_data,
        district_data=district_data,commodity_data=commodity_data,
        commodity_data_variety=commodity_data_variety)
...
def market_result():
    ...
    if variety.lower() !='all':
        sql = """with net_prices as (
            select dis.d2_id as district_id, 
            round(avg(pt.modal_price_avg) - 0.6*avg(dis.dist))::int as net_price, 
            avg(dis.dist)::int as dist
            ...
            inner join price_trend pt on m.market_id = pt.market_id and pt.variety_id in
            (select variety_id from commodity,variety 
            where commodity.commodity_id = variety.commodity_id and commodity_name = %s and 
            variety_name = %s)and pt.month = %s group by dis.d2_id)
        ...
        sql = """with net_prices as(
            ...
            inner join distances dis on dis.d1_id = d.district_id and dis.dist < 500\
            inner join market m on dis.d2_id = m.district_id\
            inner join price_trend pt on m.market_id = pt.market_id and pt.variety_id\
            in (select variety_id from commodity,variety where\
            commodity.commodity_id = variety.commodity_id and commodity_name = %s)and\
            pt.month = %s group by dis.d2_id)
            select np.*, d.district_name, s.state_name
            from (select * from net_prices order by net_price desc limit 10) np
            inner join district d on d.district_id = np.district_id
            inner join state s on d.state_id = s.state_id
...
def get_varieties():
    ...
    if commodity_name == None:
        ...
        sql = """ select variety_id,variety_name from variety where commodity_id in\
            (select commodity_id from commodity where commodity_name = %s)"""
        con.commit()
...
def add_price_data():
    ...
    market_data = []
    return render_template('add.html',state_data = state_data,\
        district_data=district_data,commodity_data=commodity_data,
...
def insert_data():
    ...
    sql = """insert into transaction_prices(market_id,transaction_date,\
        price,variety_id,comment) values (%s,%s,%s,%s,%s);"""
